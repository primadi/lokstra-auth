...
import (
	...
	coredto "github.com/primadi/lokstra-auth/pkg/dto/core"
...
type SessionStore interface {
	...
	Create(ctx context.Context, session *coredomain.Session) error

	// Get retrieves a session by ID
	Get(ctx context.Context, id string) (*coredomain.Session, error)

	// GetByToken retrieves a session by token hash
	GetByToken(ctx context.Context, tokenHash string) (*coredomain.Session, error)

	// Update updates an existing session
	Update(ctx context.Context, session *coredomain.Session) error
	...
	ListByUser(ctx context.Context, tenantID, userID string) ([]*coredomain.Session, error)

	// ListActive lists all active sessions for a user
	ListActive(ctx context.Context, tenantID, userID string) ([]*coredomain.Session, error)
...
type RefreshTokenStore interface {
	...
	Create(ctx context.Context, token *coredomain.RefreshToken) error

	// Get retrieves a refresh token by ID
	Get(ctx context.Context, id uuid.UUID) (*coredomain.RefreshToken, error)

	// GetByToken retrieves a refresh token by token hash
	GetByToken(ctx context.Context, tokenHash string) (*coredomain.RefreshToken, error)

	// Update updates an existing refresh token
	Update(ctx context.Context, token *coredomain.RefreshToken) error
	...
	// RevokeFamily revokes all tokens in a token family (for rotation detection)
	RevokeFamily(ctx context.Context, tokenFamily string) error

	// Delete deletes a refresh token
	Delete(ctx context.Context, id uuid.UUID) error

	// ListByUser lists all refresh tokens for a user
	ListByUser(ctx context.Context, tenantID, userID string) ([]*coredomain.RefreshToken, error)

	// ListBySession lists all refresh tokens for a session
	ListBySession(ctx context.Context, sessionID string) ([]*coredomain.RefreshToken, error)

	// RevokeAllByUser revokes all refresh tokens for a user
	RevokeAllByUser(ctx context.Context, tenantID, userID string) error

	// CountActiveByUser counts active refresh tokens for a user
	CountActiveByUser(ctx context.Context, tenantID, userID string) (int, error)

	// RevokeOldestByUser revokes the oldest active refresh token for a user
	RevokeOldestByUser(ctx context.Context, tenantID, userID string) error

	// CleanupExpired deletes expired tokens
	CleanupExpired(ctx context.Context) (int, error)

	// Exists checks if a refresh token exists
...
type AuditLogStore interface {
	...
	List(ctx context.Context, req *coredto.ListAuditLogsRequest) ([]*coredomain.AuditLog, error)

	// Count counts audit logs matching filters
	Count(ctx context.Context, req *coredto.ListAuditLogsRequest) (int64, error)
