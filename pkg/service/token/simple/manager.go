...
type Manager struct {
	// @Inject "cfg:auth.simple_token_manager"
	Config *Config
...
// Revoke implements [token.TokenManager].
func (m *Manager) Revoke(ctx context.Context, refreshToken string) error {
	// do nothing for simple token manager
	return nil
}

// GenerateRefreshToken implements [token.TokenManager].
func (m *Manager) GenerateRefreshToken(ctx context.Context, claims token.Claims) (*token.Token, error) {
	// For simplicity, use the same method as Generate
	return m.Generate(ctx, claims)
}

// Refresh implements [token.TokenManager].
func (m *Manager) Refresh(ctx context.Context, refreshToken string) (*token.Token, error) {
	// For simplicity, just verify the refresh token and generate a new access token
	verResult, err := m.Verify(ctx, refreshToken)
	if err != nil {
		return nil, err
	}
	if !verResult.Valid {
		return nil, verResult.Error
	}

	// Generate a new access token with the same claims
	return m.Generate(ctx, verResult.Claims)
}
...
var _ token.TokenManager = (*Manager)(nil)
