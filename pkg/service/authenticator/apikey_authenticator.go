...
import (
	...
	"github.com/primadi/lokstra-auth/pkg/domain/credential/apikey"
...
func (a *APIKeyAuthenticator) Authenticate(ctx *request.Context, authCtx *domain.AuthContext,
	creds domain.Credentials) (*domain.AuthenticationResult, error) {
	...
	// Check if revoked
	if key.DeletedAt != nil || key.Status != "active" {
		return &domain.AuthenticationResult{
			Success:  false,
			TenantID: authCtx.TenantID,
			AppID:    authCtx.AppID,
