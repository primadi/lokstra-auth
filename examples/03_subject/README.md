...
**Run**:
```bash
go run examples/03_subject/01_simple/main.go
```
...
**Run**:
```bash
go run examples/03_subject/02_enriched/main.go
```
...
**Run**:
```bash
go run examples/03_subject/03_cached_store/main.go
```
...
```bash
# PowerShell
Get-ChildItem examples/03_subject/*/main.go | ForEach-Object { 
...
# Bash
for dir in examples/03_subject/*/; do
...
```go
import (
    "github.com/primadi/lokstra-auth/03_subject/cached"
    "time"
)
