...
import (
	...
	identitypkg "github.com/primadi/lokstra-auth/identity"
...
func main() {
	...
	customEnricher := enriched.NewCustomEnricher(func(ctx context.Context, ic *identitypkg.IdentityContext) error {
		// Add custom business logic
		if ic.HasRole("admin") {
			ic.Metadata["is_super_user"] = true
			ic.Metadata["ui_theme"] = "admin-dark"
		}

		if ic.HasPermission("users:*") {
			ic.Metadata["can_manage_users"] = true
		}

		// Add computed fields
		ic.Metadata["permission_count"] = len(ic.Permissions)
		ic.Metadata["role_count"] = len(ic.Roles)
