...
func (s *InMemoryAppKeyStore) Revoke(ctx context.Context, keyID string) error {
	s.mu.Lock()
	defer s.mu.Unlock()
...
func (s *InMemoryAppKeyStore) Delete(ctx context.Context, id string) error {
	s.mu.Lock()
	defer s.mu.Unlock()
	...
	delete(s.keys, id)
	delete(s.keysByKeyID, key.KeyID)
	return nil
}
