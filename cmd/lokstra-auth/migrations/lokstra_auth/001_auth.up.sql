...
-- 4. users =
-- 5. user_identities -
-- 6. credential_providers -
-- 7. user_apps -
-- 8. app_keys -
-- 9. roles
-- 10. user_roles
-- 11. permissions
-- 12. permission_compositions
-- 13. role_permissions
-- 14. user_permissions
-- 15. policies
-- 16. tenant_ownership_history
-- 17. transfer_tenant_ownership function
-- 18. is_tenant_owner function
-- 19. get_tenant_owner function
-- 20. audit_logs
-- 21. refresh_token -
-- 22. session -
-- 23. cleanup_audit_logs function
-- 24. validate_tenant_owner function
-- ============================================================
...
CREATE TABLE IF NOT EXISTS tenants (
    ...
    domain VARCHAR(255),
    owner_id VARCHAR(255),  -- Nullable to avoid circular dependency with users table
...
CREATE UNIQUE INDEX idx_users_one_owner_per_tenant ON users(tenant_id) WHERE is_tenant_owner = TRUE AND deleted_at IS NULL;
...
-- Optimize app_keys queries
CREATE INDEX idx_app_keys_tenant_app_revoked ON app_keys(tenant_id, app_id, revoked, expires_at);
CREATE INDEX idx_app_keys_key_id_revoked ON app_keys(key_id, revoked);

-- Optimize user_apps for access checks
CREATE INDEX idx_user_apps_user_status ON user_apps(tenant_id, user_id, status);
CREATE INDEX idx_user_apps_app_status ON user_apps(tenant_id, app_id, status);
...
CREATE TABLE IF NOT EXISTS tenant_ownership_history (
    ...
    transferred_by_tenant VARCHAR(255) NOT NULL,
    reason TEXT,
...
BEGIN
    ...
    FROM users
    WHERE tenant_id = p_tenant_id AND id = p_new_owner_id;
...
CREATE OR REPLACE FUNCTION get_tenant_owner(p_tenant_id VARCHAR(255))
RETURNS TABLE (
    user_id VARCHAR(255),
...
BEGIN
    ...
    FROM users u
    JOIN tenants t ON t.owner_id = u.id AND t.id = u.tenant_id
...
-- Optimize audit_logs time-series queries
CREATE INDEX idx_audit_logs_tenant_created ON audit_logs(tenant_id, created_at DESC);
CREATE INDEX idx_audit_logs_user_created ON audit_logs(tenant_id, user_id, created_at DESC);
...
CREATE TABLE IF NOT EXISTS sessions (
    ...
    CONSTRAINT fk_sessions_user
        ...
        REFERENCES users(tenant_id, id)
        ON DELETE CASCADE,
    CONSTRAINT fk_sessions_app
        FOREIGN KEY (tenant_id, app_id)
        REFERENCES apps(tenant_id, id)
        ON DELETE CASCADE
);
...
COMMENT ON TABLE sessions IS 'Active user sessions for tracking and management';
COMMENT ON COLUMN sessions.session_token_hash IS 'Hashed session token for security';
COMMENT ON COLUMN sessions.status IS 'Session status: active, expired, revoked';

-- ============================================================
-- Refresh Tokens Table (OAuth2/JWT Token Management)
-- ============================================================
...
CREATE TABLE IF NOT EXISTS refresh_tokens (
    ...
    token_family VARCHAR(255),  -- For rotation tracking
    rotated_from_id UUID,  -- Previous token in the family
...
CREATE INDEX idx_refresh_tokens_family ON refresh_tokens(token_family) WHERE token_family IS NOT NULL;
...
CREATE INDEX idx_refresh_tokens_rotated_from ON refresh_tokens(rotated_from_id) WHERE rotated_from_id IS NOT NULL;

COMMENT ON TABLE refresh_tokens IS 'Refresh tokens for OAuth2/JWT authentication';
COMMENT ON COLUMN refresh_tokens.token_hash IS 'SHA3-256 hashed refresh token';
COMMENT ON COLUMN refresh_tokens.token_family IS 'Token family ID for rotation detection';
COMMENT ON COLUMN refresh_tokens.status IS 'Token status: active, used, expired, revoked';
COMMENT ON COLUMN refresh_tokens.rotated_from_id IS 'Previous token ID in rotation chain (for grace period tracking)';
...
BEGIN
    -- Skip validation if owner_id is NULL (allows creating tenant first, then user, then linking)
