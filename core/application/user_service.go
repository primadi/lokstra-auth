...
// @RouterService name="user-service", prefix="${api-auth-prefix:/api/auth}/core/tenants/{tenant_id}/users", middlewares=["recovery", "request_logger", "auth"]
...
func (s *UserService) CreateUser(ctx *request.Context, req *domain.CreateUserRequest) (*domain.User, error) {
	...
	existing, err := s.Store.GetByUsername(ctx, tenant.ID, req.Username)
	...
	existing, err = s.Store.GetByEmail(ctx, tenant.ID, req.Email)
	if err == nil && existing != nil {
...
func (s *UserService) GetUser(ctx *request.Context, req *domain.GetUserRequest) (*domain.User, error) {
	user, err := s.Store.Get(ctx, req.TenantID, req.ID)
	if err != nil {
...
func (s *UserService) GetUserByUsername(ctx *request.Context, req *domain.GetUserByUsernameRequest) (*domain.User, error) {
	user, err := s.Store.GetByUsername(ctx, req.TenantID, req.Username)
	if err != nil {
...
func (s *UserService) GetUserByEmail(ctx *request.Context, req *domain.GetUserByEmailRequest) (*domain.User, error) {
	user, err := s.Store.GetByEmail(ctx, req.TenantID, req.Email)
	if err != nil {
...
func (s *UserService) UpdateUser(ctx *request.Context, req *domain.UpdateUserRequest) error {
	...
	dUser, err := s.Store.Get(ctx, req.TenantID, req.ID)
	if err != nil {
	...
	if req.Username != "" && req.Username != dUser.Username {
		...
		existing, err := s.Store.GetByUsername(ctx, req.TenantID, req.Username)
		if err == nil && existing != nil {
	...
	// Save to store
	if err := s.Store.Update(ctx, dUser); err != nil {
...
func (s *UserService) ListUsers(ctx *request.Context, req *domain.ListUsersRequest) ([]*domain.User, error) {
	users, err := s.Store.List(ctx, req.TenantID)
	if err != nil {
...
func (s *UserService) AssignUserToApp(ctx *request.Context, req *domain.AssignUserToAppRequest) error {
	// Verify user exists
	_, err := s.Store.Get(ctx, req.TenantID, req.UserID)
	if err != nil {
		return fmt.Errorf("user not found: %w", err)
	}

	// Verify app exists
	_, err = s.AppService.GetApp(ctx, &domain.GetAppRequest{
		TenantID: req.TenantID,
		ID:       req.AppID,
	})
	if err != nil {
...
func (s *UserService) SetPassword(ctx *request.Context, req *domain.SetPasswordRequest) error {
	// Verify user exists
	_, err := s.Store.Get(ctx, req.TenantID, req.ID)
	if err != nil {
		return fmt.Errorf("user not found: %w", err)
	}
	...
	if err := s.Store.SetPassword(ctx, req.TenantID, req.ID, passwordHash); err != nil {
...
func (s *UserService) RemovePassword(ctx *request.Context, req *domain.RemovePasswordRequest) error {
	// Verify user exists
	_, err := s.Store.Get(ctx, req.TenantID, req.ID)
	if err != nil {
		return fmt.Errorf("user not found: %w", err)
	...
	if err := s.Store.RemovePassword(ctx, req.TenantID, req.ID); err != nil {
