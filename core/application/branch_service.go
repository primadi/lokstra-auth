...
// @RouterService name="branch-service", prefix="${api-auth-prefix:/api/auth}/core/tenants/{tenant_id}/apps/{app_id}/branches", middlewares=["recovery", "request_logger", "auth"]
type BranchService struct {
	// @Inject "@store.branch-store"
	Store repository.BranchStore
	// @Inject "app-service"
...
func (s *BranchService) CreateBranch(ctx *request.Context, req *domain.CreateBranchRequest) (*domain.Branch, error) {
	...
	existing, err := s.Store.Get(ctx, req.TenantID, req.AppID, req.BranchID)
...
func (s *BranchService) GetBranch(ctx *request.Context, req *domain.GetBranchRequest) (*domain.Branch, error) {
	branch, err := s.Store.Get(ctx, req.TenantID, req.AppID, req.ID)
	if err != nil {
...
func (s *BranchService) UpdateBranch(ctx *request.Context, req *domain.UpdateBranchRequest) error {
	...
	dBranch, err := s.Store.Get(ctx, req.TenantID, req.AppID, req.ID)
	if err != nil {
	...
	// Save to store
	if err := s.Store.Update(ctx, dBranch); err != nil {
...
func (s *BranchService) DeleteBranch(ctx *request.Context, req *domain.DeleteBranchRequest) error {
	// Check if branch exists
	exists, err := s.Store.Exists(ctx, req.TenantID, req.AppID, req.ID)
	if err != nil {
	...
	// Delete from store
	if err := s.Store.Delete(ctx, req.TenantID, req.AppID, req.ID); err != nil {
...
func (s *BranchService) ListBranches(ctx *request.Context, req *domain.ListBranchesRequest) ([]*domain.Branch, error) {
	if req.Type != "" {
		branches, err := s.Store.ListByType(ctx, req.TenantID, req.AppID, req.Type)
		if err != nil {
	...
	branches, err := s.Store.List(ctx, req.TenantID, req.AppID)
	if err != nil {
...
func (s *BranchService) ActivateBranch(ctx *request.Context, req *domain.ActivateBranchRequest) error {
	...
	if err := s.Store.Update(ctx, branch); err != nil {
