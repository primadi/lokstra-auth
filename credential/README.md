...
**Basic Authenticator:**
- `CredentialValidator` - Validates username/password complexity (see `credential/basic/contract.go`)
- `UserProvider` - Retrieves user data from storage

**Passkey Authenticator:**
- `CredentialStore` - Manages WebAuthn credentials (see `credential/passkey/store.go`)
...
```go
import "github.com/primadi/lokstra-auth/credential/basic"
...
```go
import "github.com/primadi/lokstra-auth/credential/oauth2"
...
```go
import "github.com/primadi/lokstra-auth/credential/passwordless"
...
```go
import "github.com/primadi/lokstra-auth/credential/apikey"
...
**Layer 01 Complete** ✅  
Continue to [Layer 02: Token](../token/README.md) for JWT token management.
