...
Subject resolution is the process of converting raw authentication claims (from tokens) into a structured `rbac` entity representing an authenticated user, service, or device.
...
Transforms token claims into a `rbac` entity.
...
**Basic Usage:**
```go
import "github.com/primadi/lokstra-auth/identity/simple"
...
**Basic Usage:**
```go
import "github.com/primadi/lokstra-auth/identity/enriched"
...
**Cached Resolver:**
```go
import "github.com/primadi/lokstra-auth/identity/cached"
...
**Basic Usage:**
```go
import subject "github.com/primadi/lokstra-auth/rbac"
...
See `examples/rbac/` folder for complete examples:
...
```bash
cd examples/rbac/03_cached_store
go run main.go
```
...
**Layer 03 Complete** ✅  
Continue to [Layer 04: Authorization](../authz/README.md) for access control.
